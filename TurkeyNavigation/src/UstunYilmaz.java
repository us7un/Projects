...
public class UstunYilmaz {
    public static void main(String[] args) throws FileNotFoundException {
        String coordinateFileName = "city_coordinates.txt";
        String connectionFileName = "city_connections.txt";
        ...
        Scanner scanCoordinates = new Scanner(coordinateFile);
        while (scanCoordinates.hasNextLine()) {
            String[] line = scanCoordinates.nextLine().split(", ");
            cityNames.add(line[0]);
            ArrayList<Integer> tuple = new ArrayList<>();
            tuple.add(Integer.parseInt(line[1]));
            tuple.add(Integer.parseInt(line[2]));
            coordinates.add(tuple);
        }
        scanCoordinates.close();

        for (int i = 0; i < cityNames.size(); i++) {
            cities.add(new City(cityNames.get(i), coordinates.get(i).getFirst(), coordinates.get(i).getLast(), i));
        }

        Scanner scanConnections = new Scanner(connectionFile);
        while (scanConnections.hasNextLine()) {
            String[] line = scanConnections.nextLine().split(",");
            ArrayList<City> tuple = new ArrayList<>();
        ...
        // StdDraw ----------------------------------------------------------------------------------------------------------------
        int pictureWidth = 2377;
        int pictureHeight = 1055;
        int canvasWidth = (int) (pictureWidth / 1.5);
        int canvasHeight = (int) (pictureHeight / 1.5);
        StdDraw.setCanvasSize(canvasWidth, canvasHeight);
        StdDraw.setXscale(0, pictureWidth);
        StdDraw.setYscale(0, pictureHeight);
        StdDraw.enableDoubleBuffering();
        StdDraw.picture(pictureWidth / 2.0, pictureHeight / 2.0, "map.png", pictureWidth, pictureHeight);
